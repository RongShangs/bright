...
![当前控制面板界面示意图](docs/panel-preview.svg)

*界面示意图：左侧显示亮度状态，中间调节亮度，右侧控制 AOD 和系统接管；外观随系统深浅色模式切换。*

## ✨ 核心特性

- **垂直亮度滑块** — 直观的滑块控件，拖动即可调节背屏亮度（手动最低 10）
...
- **永不息屏** — 将背屏超时设为最大值，适用于灯牌、氛围灯；不会主动因超时休眠，但不能保证按电源键后仍保持亮屏。常亮会增加耗电与烧屏风险
- **控制中心磁贴** — 下拉通知栏快速打开控制面板
- **控制中心入口稳定** — 磁贴打开的面板位于应用自身窗口；桌面图标仍使用非焦点悬浮窗
...
| 权限 | Root（Magisk / KernelSU）+ 显示在其他应用上层 |
...
- **控制面板**：控制中心磁贴打开时后方应用会暂时暂停，关闭面板后恢复；桌面图标入口使用悬浮窗，需授予悬浮窗权限
- **锁定模式**：`chmod 444` 设为只读，系统无法自动调节
- **系统控制**：`chmod 644` 恢复可写，系统重新接管亮度调节
- **AOD 控制**：通过 `settings put secure rear_doze_always_on` 开关背屏息屏显示
- **永不息屏**：通过 `subscreen_display_time` 控制背屏超时，独立于亮度锁定和 AOD；“恢复系统控制”会同时恢复正常超时。卸载应用前请先恢复系统控制

### 底层优化

- **Su 进程复用**：面板打开期间复用 su 进程，通过 marker 标记命令输出边界；面板关闭或恢复系统控制后立即释放
...
Hyper OS 4 中系统会持续覆盖背屏亮度值，且 App 进程可能被系统冻结。为此引入**独立 C 守护进程**，静态编译，即使 App 被杀依然驻留后台守护亮度。

> 💡 **几乎零占用**：守护进程仅 **128KB 内存**、**0.0% CPU**、纯休眠等待，对日常使用毫无感知，可常驻后台放心使用。
...
下载 [最新版 v1.6.5](https://github.com/RongShangs/bright/releases/tag/v1.6.5)，旧版本及详细变更请查看 [Releases](https://github.com/RongShangs/bright/releases) 和 [CHANGELOG.md](CHANGELOG.md)。
...
- v1.6.5 APK SHA-256：`BE617A551F477DB816567CF953CE227875812E51D900DCBCB1317BA70FECA470`
